...
| **行业分类(全系统口径)** | ✅ Tushare | `index_classify`+`index_member_all`(申万SW2021) | **2026-06-20 升级**：`get_stock_basic().industry` 覆盖为**申万二级**(131/134个·够细：半导体/消费电子/元件/光学分开)，原Tushare值留 `industry_src`、申万一级在 `industry_l1`(供上卷)；映射失败优雅回退。所有按 industry 聚合的板块分析(行业资金/宽表/全景看板/广度雷达/同类回测/选股池)统一升级。选二级因：炒股口径(对标同花顺/吴川细分)+每板块股票够多统计稳 |
| 行业资金流 | ✅ Tushare(聚合) | 按 `industry`(=申万二级) 聚合 `moneyflow` | industry_flow 页；非 Akshare |
