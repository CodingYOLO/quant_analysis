...
_IND: list[MetricDef] = [
    ...
    MetricDef(
        ...
        unit="℃", direction=0, source="noaa", api="CPC oni.ascii.txt",
        lag_days=36, sort_order=80,
        note="✅20260804 腾讯云生产直连实测可达(HTTP 200·23KB·2.7s·919行·格式统一无缺失)。"
             "value=ANOM(Niño3.4三月滑动海温距平)·SEAS中心月月末为as_of。"
             "lag_days=36 由真实发布时点定：MJJ(中心月6/30) Last-Modified 8/3 GMT=滞后34天·留2天余量。"
             "≥+0.5℃连5个滑动季=厄尔尼诺·≤-0.5℃=拉尼娜；direction=0——ENSO对A股无单边多空含义，"
             "传导必须经天气敏感农产品期货(fut_sr/fut_p/fut_cf/fut_ap)的价格印证。"
             "近几季数值随ERSST小幅修订·每晚全量重取覆盖写自然吸收。",
    ),
]
...
EXPLAIN: dict[str, str] = {
    ...
    "enso_oni": "NOAA海洋尼诺指数(ONI)——Niño3.4海区三个月滑动平均海温距平(℃)·厄尔尼诺/拉尼娜的"
                "**官方判据**：≥+0.5℃连续5个滑动季=厄尔尼诺·≤-0.5℃=拉尼娜。链条位置：气候异常→"
                "农产品供给冲击预期(印泰甘蔗/东南亚棕榈/新疆美棉)→期货价→A股种植/糖业/油脂板块。"
                "怎么读：只定气候基调·配下方农产品期货一起看——ONI走高而糖价棕榈油不动=天气题材虚炒。"
                "陷阱：月频且滞后约36天发布(三月滑动窗走完次月初CPC才更新)·不做择时；"
                "拉尼娜(负值)同样是供给冲击(南美大豆减产利多豆粕)·正负两端都值得警觉·别当单边指标。",
}
