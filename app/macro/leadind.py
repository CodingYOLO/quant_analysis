...
CHAINS: list[dict] = [
    ...
    {"label": "🌦 天气敏感农产品",
     "codes": ["enso_oni", "fut_sr", "fut_p", "fut_cf", "fut_ap"],
     "logic": "链条：ONI(厄尔尼诺/拉尼娜官方判据·月频定基调) → 农产品供给冲击预期 → 期货价"
              "(=天气信息的日频聚合)：糖(印泰干旱)、棕榈油(东南亚)、棉花(新疆/美棉)、苹果(花期冻害)。"
              "ONI异常而这些价格不动=天气题材虚炒；价格动而ONI正常=另有驱动(政策/需求)。"},
...
IMPACT: dict[str, dict] = {
    ...
    "enso_oni": {"up_good": "走高(厄尔尼诺)=糖/棕榈油减产预期·关注制糖油脂链是否印证",
                 "up_bad": "走低(拉尼娜)=南美大豆/东南亚多雨风险·同为供给冲击非利好"},
