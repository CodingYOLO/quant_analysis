...
def _build_note_prompt(facts: str) -> str:
    ...
    return (
        "你是资深A股量化分析师。下面是对一只票做的【反向策略扫描已算好结果】："
        "把多个技术信号在该票近窗口内的历史表现都跑了一遍。请用 2-3 句话给一个**有洞察、敢下结论**的总评："
        "这只票最吃哪类打法、什么情况下用、要注意什么——别只复述数字。\n\n"
        "底线：\n"
        "1. 只引用下方给出的数字，绝不臆造或推算新数字；对样本偏少(n<8)的信号显式提示「仅参考」；这是历史统计、历史≠未来。\n"
        "2. 不编造新的胜率/概率、不打包票「必涨」、不给「全仓买入」式硬指令；但可以明确说它最吃哪种打法、相对更适合追涨还是低吸。\n"
        "3. 把「信号表现」和「股性」串起来讲（如妖性强→更吃突破/打板；趋势票→更吃均线跟随）。\n"
        "4. 只输出点评正文本身，不要分点、不要前后缀、不要代码块。\n\n"
        f"数据：\n{facts}"
    )
