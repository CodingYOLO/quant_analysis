...
def build_prompt(name: str, code: str, signal_label: str, facts: str) -> str:
    ...
    return (
        ...
        "“这只票 + 这个打法在当前环境下值不值得做”。\n\n"
        "要求（专业·敢下结论·但诚实）：\n"
        "1. 只引用下方给出的数字，绝不臆造或推算新数字；对样本偏小显式标「仅参考」；历史≠未来；\n"
        "2. **给出明确结论**：这票+这个打法当前值不值得做、强在哪弱在哪、关键风险是什么——别和稀泥、别把判断全推给用户；\n"
        "3. 但不编造新的胜率/成功率、不打包票「必涨/稳赚」、不给「全仓买入/清仓」式硬下单指令；最终买卖与仓位由用户定、风险自负；\n"
