...
def node_risk_debate(state: PipelineState) -> PipelineState:
    ...
    for candidate in state.candidates:
        ...
        if soft_veto:
            candidate.risk_flags.append(f"⚠️ 空头否决: {soft_reason}")
            candidate.debate.verdict = "否决"
            candidate.debate.verdict_reason = soft_reason
            candidate.debate.bull_points = bull_points
            candidate.debate.bear_points = bear_points
            vetoed.append(candidate)
            logger.info("  [软否决] %s — %s", candidate.name, soft_reason)
        else:
...
def _build_stock_info(candidate: Candidate, decay_industries: set[str]) -> str:
    ...
    return (
        ...
        f"龙虎榜：{'是' if f.lhb_flag else '否'}　千股千评得分：{f.comment_score:.0f}"
    )
...
def _llm_debate(
    ...
    失败时返回默认值，不阻断流程。
    """
    stock_info = _build_stock_info(candidate, decay_industries)
    llm = LLMClient()
