...
def _append_llm_review(lines: list, state: PipelineState) -> None:
    ...
    try:
        ...
        prompt = (
            ...
            f"**敢下判断**：今天是进攻还是防守、主线是谁、候选里哪只更强、风险在哪——给鲜明观点，别只罗列、别和稀泥。\n"
            f"严禁编造未出现的公司/数字/事件；不编胜率%、不打包票「必涨」；判断说清是基于现有数据的观点。\n"
            f"结构（连贯成段，不分点标题）：①大盘环境与赚钱效应(明确给进攻/防守判断) ②今日主线板块 "
            f"③为何选中这些候选（结合板块热度/资金/多路验证，横向点评同主题谁强谁弱）④风险提示（含避雷）。\n\n"
