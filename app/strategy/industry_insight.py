...
def _theme_group(df, theme: str, provider):
    ...
    canon, subs = _resolve_parent(theme)                          # 宽口径：申万一级/俗称→二级并集（医药行业→整片医药）
    if subs:
        g = df[ind.isin(subs)]
        if not g.empty:
            return g, f"申万一级「{canon}」（{len(subs)}个二级并集）"
...
# 申万一级 → 其下申万二级列表（宽口径主题的成分并集·基于因子表实际二级名·SW2021）。
# 用户输"医药行业/医药/大健康"这类宽主题时，落到整片板块的全部个股，而非只碰巧含子串的一个二级。
_SW_L1 = {
    "医药生物": ["化学制药", "中药Ⅱ", "生物制品", "医药商业", "医疗器械", "医疗服务", "医疗保健"],
    "电子": ["半导体", "元件", "光学光电子", "消费电子", "其他电子Ⅱ", "电子化学品Ⅱ"],
    "电力设备": ["电池", "光伏设备", "风电设备", "电网设备", "电机Ⅱ", "电气设备", "其他电源设备Ⅱ"],
    "有色金属": ["工业金属", "贵金属", "小金属", "能源金属", "金属新材料"],
    "汽车": ["乘用车", "商用车", "汽车零部件", "汽车配件", "汽车服务", "摩托车及其他"],
    "机械设备": ["专用设备", "通用设备", "自动化设备", "工程机械", "机械基件", "轨交设备Ⅱ"],
    "计算机": ["软件开发", "IT服务Ⅱ", "计算机设备"],
    "国防军工": ["航天装备Ⅱ", "航空装备Ⅱ", "地面兵装Ⅱ", "航海装备Ⅱ", "军工电子Ⅱ"],
    "食品饮料": ["白酒Ⅱ", "非白酒", "饮料乳品", "休闲食品", "食品加工", "调味发酵品Ⅱ"],
    "家用电器": ["白色家电", "黑色家电", "小家电", "厨卫电器", "家电零部件Ⅱ", "其他家电Ⅱ", "照明设备Ⅱ"],
    "银行": ["国有大型银行Ⅱ", "股份制银行Ⅱ", "城商行Ⅱ", "农商行Ⅱ"],
    "非银金融": ["证券Ⅱ", "保险Ⅱ", "多元金融"],
    "基础化工": ["化学制品", "化学原料", "化学纤维", "化工原料", "农化制品", "塑料", "橡胶"],
    "房地产": ["房地产开发", "房地产服务"],
    "传媒": ["出版", "影视院线", "广告营销", "数字媒体", "游戏Ⅱ", "电视广播Ⅱ"],
    "通信": ["通信服务", "通信设备"],
    "钢铁": ["普钢", "特钢Ⅱ", "冶钢原料"],
    "煤炭": ["煤炭开采", "焦炭Ⅱ"],
    "建筑装饰": ["基础建设", "房屋建设Ⅱ", "专业工程", "装修装饰Ⅱ", "工程咨询服务Ⅱ"],
    "建筑材料": ["水泥", "玻璃玻纤", "装修建材", "非金属材料Ⅱ"],
    "农林牧渔": ["种植业", "养殖业", "渔业", "林业Ⅱ", "饲料", "农产品加工", "动物保健Ⅱ", "农业综合Ⅱ"],
    "纺织服饰": ["纺织制造", "服装家纺", "饰品"],
    "美容护理": ["化妆品", "医疗美容", "个护用品"],
    "石油石化": ["油气开采Ⅱ", "油服工程", "炼化及贸易"],
    "公用事业": ["电力", "燃气Ⅱ", "新型电力"],
    "交通运输": ["物流", "铁路公路", "航运港口", "航空机场"],
    "环保": ["环保设备Ⅱ", "环境治理"],
    "商贸零售": ["一般零售", "专业连锁Ⅱ", "互联网电商", "贸易Ⅱ", "旅游零售Ⅱ"],
    "社会服务": ["旅游及景区", "酒店餐饮", "教育", "专业服务", "体育Ⅱ"],
    "轻工制造": ["家居用品", "包装印刷", "造纸", "文娱用品"],
}

# 俗称/别名 → 上面的规范申万一级名（宽口径识别：医药/大健康→医药生物 等）
_PARENT_ALIAS = {
    "医药": "医药生物", "大健康": "医药生物", "医疗": "医药生物", "生物医药": "医药生物", "医药生物": "医药生物",
    "新能源": "电力设备", "电新": "电力设备", "锂电池": "电力设备", "光伏风电": "电力设备",
    "有色": "有色金属", "金属": "有色金属",
    "军工": "国防军工", "国防": "国防军工",
    "化工": "基础化工",
    "地产": "房地产",
    "券商": "非银金融", "非银": "非银金融", "保险": "非银金融", "金融": "非银金融",
    "家电": "家用电器",
    "石化": "石油石化", "石油": "石油石化",
    "公用": "公用事业",
    "交运": "交通运输", "物流运输": "交通运输",
    ...
    "纺服": "纺织服饰", "服装": "纺织服饰",
    "美护": "美容护理", "化妆品护理": "美容护理",
    "食品": "食品饮料", "饮料": "食品饮料", "白酒板块": "食品饮料",
    "建筑": "建筑装饰", "建材": "建筑材料",
}


def _resolve_parent(theme: str) -> tuple[str | None, list[str]]:
    """宽口径主题（申万一级名/俗称/带"行业/板块"后缀）→ (规范一级名, 申万二级列表)。命中不了→(None,[])。"""
    import re
    key = (theme or "").strip()
    key = re.sub(r"(行业|板块|概念|产业|板|股)$", "", key) or (theme or "").strip()
    canon = key if key in _SW_L1 else _PARENT_ALIAS.get(key)
    if canon and canon in _SW_L1:
        return canon, _SW_L1[canon]
    return None, []
...
def gen_quiz(theme: str, card: str, client=None) -> dict:
    ...
    # 出题用 flash（deepseek-chat）：据现成卡片提问不需最强推理·比 pro 快~20s（卡片本身已 pro 生成）
