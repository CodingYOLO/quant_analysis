...
def watch_meta() -> dict:
    """自选/持仓元信息 {ts_code: {name, is_holding, stop_loss, owners:[...]}}（集合竞价/盯盘异动用）。

    多归属：同一只票可同时在不同人列表里 → 按 ts_code 聚合，owners 收关注它的人(分人推送路由用)；
    is_holding=任一人持仓即真(影响穿透级别)；name/stop_loss 取持仓那条优先。
    """
    from app.strategy import db
    meta: dict = {}
    for w in db.get_watchlist():
        code = w["ts_code"]
        m = meta.setdefault(code, {"name": w.get("name", ""), "is_holding": False,
                                   "stop_loss": w.get("stop_loss"), "owners": []})
        owner = w.get("owner", "me")
        if owner not in m["owners"]:
            m["owners"].append(owner)
        if w.get("is_holding"):                      # 任一人持仓 → 标持仓(并以持仓条的名/止损为准)
            m["is_holding"] = True
            m["name"] = w.get("name", "") or m["name"]
            m["stop_loss"] = w.get("stop_loss")
    return meta
