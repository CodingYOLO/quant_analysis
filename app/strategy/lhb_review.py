...
def _short_seat(ex: str) -> str:
    """营业部全名 → 精简显示名（去券商前缀/通用后缀，留辨识度高的网点部分）。"""
    s = str(ex or "")
    for kw in ("股份有限公司", "有限责任公司", "有限公司", "证券营业部", "营业部"):
        s = s.replace(kw, "")
    if "证券" in s:                       # 去掉"XX证券"前缀，保留网点（游资辨识靠网点）
        s = s.split("证券", 1)[1] or s
    return (s or str(ex))[:14]


def _hot_relay(occ: list[dict]) -> list[dict]:
    """跨多次上榜聚合游资席位 → 谁在反复进场（接力）。纯函数·按出现天数/净买排序。

    妖股能否续命的硬证据：同一游资在多次上榜反复买入 = 接力；只来一次多是一日游。
    """
    agg: dict[str, dict] = {}
    for o in occ:
        for s in o.get("seats", []):
            if s.get("type") != "hot":            # 只聚合游资席位
                continue
            key = s.get("nickname") or s.get("name")
            a = agg.setdefault(key, {"name": s["name"], "nickname": s.get("nickname", ""),
                                     "dates": set(), "net_yi": 0.0})
            a["dates"].add(o["date"])
            a["net_yi"] += s.get("net_yi", 0.0)
    rows = [{"name": v["name"], "nickname": v["nickname"], "days": len(v["dates"]),
             "net_yi": round(v["net_yi"], 2)} for v in agg.values()]
    rows.sort(key=lambda r: (-r["days"], -r["net_yi"]))
    return rows[:8]
...
def review_stock(provider: CompositeProvider, ts_code: str, start: str, end: str) -> dict:
    ...
    for day in _candidate_days(k):
        ...
        notable = [{"name": _short_seat(s["exalter"]), "label": s["label"],
                    "nickname": s.get("nickname", ""), "type": s["type"], "net_yi": s["net_yi"]}
                   for s in seats if s["type"] != "normal"][:8]   # 具名席位(机构/游资/北向/外资)
        occ.append({
            ...
            "seat_count": len(seats), "seats": notable,
