...
@cli.command("us-map")
@click.option("--date", "trade_date", default="last", help="A股交易日，默认最近交易日")
def us_map_cmd(trade_date: str) -> None:
    """美股→A股映射刷新（早08:00 cron：抓昨夜美股收盘·保证A股开盘前可见）。
...
def run_warmup(base_date: str) -> None:
    ...
    try:
        from app.strategy.sector_mtf import build_sector_mtf, build_sector_mtf_ai
        ...
        build_sector_mtf_ai(latest, provider=prov, force=True)          # 大周期格局 AI 研判(夜间预算一次LLM)
